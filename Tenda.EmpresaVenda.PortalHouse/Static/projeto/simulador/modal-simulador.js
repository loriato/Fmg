...
Europa.Controllers.Simulador.AbrirModalSimulador = function () {
    ...
    $("#btn-aplicar-simulacao").css("display","none");
    $("#simulacoes").val("").trigger("change");
...
function DetalhamentoFinanceiroModalSimulador($scope, $compile, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder) {
    ...
    tabelaWrapper.
        setColumns([
            DTColumnBuilder.newColumn('TipoParcela').withTitle(Europa.i18n.Messages.TipoParcela).withOption('width', '10px').withOption('type', 'enum-format-TipoParcela'),
            DTColumnBuilder.newColumn('NumeroParcelas').withTitle(Europa.i18n.Messages.NumeroParcelas).withOption('width', '15px'),
            DTColumnBuilder.newColumn('ValorParcela').withTitle(Europa.i18n.Messages.ValorParcela).withOption('width', '10px').renderWith(renderMoney).withClass('dt-body-right'),
            DTColumnBuilder.newColumn('Total').withTitle(Europa.i18n.Messages.Total).withOption('width', '10px').renderWith(renderMoney).withClass('dt-body-right'),
            DTColumnBuilder.newColumn('DataVencimento').withTitle(Europa.i18n.Messages.DataVencimento).withOption('width', '15px').withOption("type", "date-format-DD/MM/YYYY")
        ])
        .setTemplateEdit([
...
Europa.Controllers.Simulador.OnChangeSimulacao = function () {
    if ($('#simulacoes').val() != "") {
        $("#btn-aplicar-simulacao").css("display", "block");
    } else {
        $("#btn-aplicar-simulacao").css("display", "none");
    }
