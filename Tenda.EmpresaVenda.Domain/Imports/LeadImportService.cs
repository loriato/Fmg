...
namespace Tenda.EmpresaVenda.Domain.Imports
{
    public class LeadImportService
    {
        ...
        public LeadImportService(long origem)
        {
            OrigemLead = origem;
        }
        ...
        public void Process(ISession session, string diretorio, ref ImportTaskDTO importTask, ImportLeadDTO importLead)
        {
            try
            {
                _session = session;
                InternalProcess(diretorio, ref importTask, importLead);
            }
            catch (Exception e)
            {
        ...
        public void InternalProcess(string diretorio, ref ImportTaskDTO importTask, ImportLeadDTO importLead)
        {
            ...
            if (validation)
            {
                var EmpresasVendas = _empresaVendaRepository.FindByIdsEmpresaVenda(importLead.IdsEmpresaVenda);
                ...
                while (rowIndex < rowCount)
                {
                    try
                    {
                        ...
                        lead.DescricaoPacote = importLead.Pacote;
                        ...
                        foreach (var empresa in EmpresasVendas)
                        {

                            var leadEmprevenda = new LeadEmpresaVenda();
                            leadEmprevenda.EmpresaVenda = empresa;
                            leadEmprevenda.Lead = lead;
