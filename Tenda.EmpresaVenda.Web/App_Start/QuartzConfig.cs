    ...
    public class QuartzConfig
    {
        ...
        public static void Config()
        {
            ...
            try
            {
                ...
                QuartzConfig config = new QuartzConfig(session);
                config.InitConfig();
            }
            catch (Exception e)
            {
        ...
        public async Task InitConfig()
        {
            ...
            QuartzConfigurationRepository repo = new QuartzConfigurationRepository(_session);

            IList<QuartzConfiguration> jobsConf = repo.Queryable().Where(reg => reg.IniciarAutomaticamente).ToList();

            foreach (QuartzConfiguration job in jobsConf)
            {
        ...
        private bool CanRunInThisServer(QuartzConfiguration quartz)
        {
            ...
            return
                ...
                (quartz.SiteExecucao.IsEmpty() || HostingEnvironment.SiteName.ToLower().Equals(quartz.SiteExecucao.ToLower())) &&
                // 
                (quartz.AplicacaoExecucao.IsEmpty() || applicationName.ToLower().Equals(quartz.AplicacaoExecucao.ToLower()));
        }
        ...
        private async Task ScheduleJob(QuartzConfiguration job)
        {
            ...
            if (classe == null)
            {
                throw new ArgumentNullException("job", string.Format("Classe não encontrada: {0}", job.CaminhoCompleto));
            }
        ...
        private void FireJobNow(QuartzConfiguration job)
        {
            ...
            {
                BusinessRuleException bre = new BusinessRuleException(MensagemServidorErrado(job));
                bre.ThrowIfHasError();
            }
            ...
            if (classe == null)
            {
                ...
                exc.AddError("Job {0} não encontrado. Esse job provavelmente pertence a outro projeto.").WithParam(job.CaminhoCompleto).Complete();
                exc.ThrowIfHasError();
            }
            ...
            if (scheduler.CheckExists(triggerKey).Result &&
                scheduler.GetCurrentlyExecutingJobs().Result.Any(x => x.Trigger.Key.Name == triggerKey.Name && x.Trigger.Key.Group == triggerKey.Group))
            {
                var exc = new BusinessRuleException();
