...
var jexcel = (function (el, options) {
    ...
    obj.createCell = function (i, j, value) {
        ...
        if (obj.options.columns[i].type == 'hidden') {
        ...
        } else if (obj.options.columns[i].type == 'decimal') {
            if (value === undefined) {
                value = "0,00";
            }
            if (value.indexOf(",") < 0) {
            ...
            td.innerHTML = value;
        } else if (obj.options.columns[i].type == 'numeric') {
            if (value === undefined) {
                value = "0";
            }
            ...
            if (obj.options.columns[i].decimalcount) {
                decimalCount = parseInt(obj.options.columns[i].decimalcount);
            }

            var valid;
            if (decimalCount > 0) {
                valid = new RegExp("^\\d*[,]?\\d{0," + decimalCount + "}$").test(value);
            } else {
                valid = /^\d*$/.test(value);
            }
            if (!valid) {
                ...
                else if (decimalCount === 0 || arr.length === 1) {
                    this.value = arr[0].replace(/\D+/,"")
                } else {
                    this.value = arr[0].replace(/\D+/,"") + "," + arr[1].substring(0, decimalCount).replace(/\D+/,"");
                }
            }
    ...
    obj.createNestedHeader = function (nestedInformation) {
        ...
        for (var i = 0; i < nestedInformation.length; i++) {
            ...
            if (!nestedInformation[i].headerId) {
                nestedInformation[i].headerId = -1;
            }
            ...
            td.setAttribute('headerId', nestedInformation[i].headerId);
    ...
    obj.openEditor = function (cell, empty, e) {
        ...
        if (cell.classList.contains('readonly') == true) {
            ...
            if (obj.options.columns[x].editor) {
                ...
                if (obj.options.columns[x].type == 'hidden') {
                ...
                } else if (obj.options.columns[x].type == 'decimal') {
                    var value = obj.options.data[y][x];
                    var editor = createEditor('input');

                    if (obj.options.columns[x].mask) {
                        $(editor).unmask().mask(obj.options.columns[x].mask, {
                            clearIfNotMatch: false,
                            reverse: true
                        });
                    } else {
                        $(editor).unmask().mask(Europa.Mask.FORMAT_DECIMAL, {
                            clearIfNotMatch: false,
                            reverse: true
                        });
                    }
                    ...
                    if (obj.options.columns[x].decimalcount) {
                        decimalCount = parseInt(obj.options.columns[x].decimalcount);
                    }

                    $(editor).on('change paste keyup', function (e) {
                        var valid;
                        if (decimalCount > 0) {
                            valid = new RegExp("^\\d*[,]?\\d{0," + decimalCount + "}$").test(this.value);
                        } else {
                        ...
                        if (valid) {
                            ...
                            console.log(this.value.replace(/\D+/,""));
                            if(arr.length === 0){
                                this.value = "0";
                            }
                            else if (decimalCount === 0 || arr.length === 1) {
                    ...
                    /*$(editor).inputFilter(function(value) {
                        return /^\d*$/.test(value);
                    });*/
                    ...
                    if (obj.options.columns[x].wordWrap != false && (obj.options.wordWrap == true || obj.options.columns[x].wordWrap == true)) {
                        ...
                        if (obj.options.columns[x].mask) {
                            var reverse = false;
                            if (obj.options.columns[x].options) {
                                reverse = obj.options.columns[x].options.reverse;
                            }
                        ...
                        if (value == '') {
                            ...
                            if (!valid) {
                                ...
                                if(arr.length === 0){
                                    ...
                                    value = arr[0].replace(/\D+/,"");
                                } else {
                                    value = arr[0].replace(/\D+/,"") + "," + arr[1].substring(0, decimalCount).replace(/\D+/,"");
                                }
                            }
        ...
        for (var j = y1; j <= y2; j++) {
            ...
            for (var i = x1; i <= x2; i++) {
                ...
                if (obj.records[j][i] && !obj.records[j][i].classList.contains('readonly') && obj.records[j][i].style.display != 'none' && breakControl == false) {
                    ...
                    posx++;
                } else if (obj.records[j][i].classList.contains('readonly')) {
                    if ((i + 1 <= x2 && obj.records[j][i + 1].style.display != 'none' && obj.records[j][i].style.display == 'none') || obj.records[j][i].style.display != 'none') {
                        posx++;
                        if (data[posy] == undefined) {
                            posx = 0;
                        } else if (data[posy][posx] == undefined) {
