...
Europa.Controllers.BookBreveLancamento.Modal.Show = function (idEmpreendimento, callbackfuncion) {
    if (idEmpreendimento === undefined) {
        alert('Parameter idEmpreendimento is required');
    }
    // Back to default state
    Europa.Controllers.BookBreveLancamento.Modal.Init();

    Europa.Controllers.BookBreveLancamento.Modal.Vars.IdBreveLancamento = idEmpreendimento;
...
Europa.Controllers.BookBreveLancamento.FilterParams = function () {
    var filtro = {
        idEmpreendimento: Europa.Controllers.BookBreveLancamento.Modal.Vars.IdBreveLancamento
    };
    return filtro;
};


DataTableApp.controller('bookEmpreendimentoTable', bookEmpreendimentoTable);
function bookEmpreendimentoTable($scope, $compile, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder) {
    ...
    .setIdAreaHeader("book_empreendimento_datatable_header")
    .setAutoInit(false)
...
Europa.Controllers.BookBreveLancamento.Upload = function () {var tipo = $("#tipo_book_select").val();
...
Europa.Controllers.BookBreveLancamento.Grid.Write = function () {
    ...
    for (index = 0; index < total; index++) {
        ...
        // Break line
        if (index % colNumber === 0) { allTemplate += "</div><div class='col-md-24'>"; }
