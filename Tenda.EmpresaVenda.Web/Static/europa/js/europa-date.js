...
Europa.Date.toFormatDate = function (value, pattern) {
    if (value) {
        var date;
        
        if(value instanceof String){
            date = Europa.Date.Parse(value);
        }else{
            date = value;
        }


        var janOffset = moment({M:0, d:1}).locale(Europa.Date.Locale).utcOffset();
        var julOffset = moment({M:6, d:1}).locale(Europa.Date.Locale).utcOffset();

        var stdOffset = Math.min(janOffset, julOffset);

        
        return moment(date).utcOffset(stdOffset).locale(Europa.Date.Locale).format(pattern);
    }
    return "";
};

Europa.Date.toFormatDate.Utc = function (date, pattern) {
    if (date) {
...
Europa.Date.Format = function (value, pattern) {
    if (value) {
        ...
        var stdOffset = Math.min(janOffset, julOffset);
        return moment(value, pattern).utcOffset(stdOffset).locale(Europa.Date.Locale).format(pattern);
    }
    return "";
}
...
Europa.Date.FormatText = function (value, pattern, format) {
    if (value) {
        ...
        var stdOffset = Math.min(janOffset, julOffset);
        return moment(value, pattern).utcOffset(stdOffset).locale(Europa.Date.Locale).format(format);
    }
    return "";
}
